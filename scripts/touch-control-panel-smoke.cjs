...
(async () => {
  ...
  try {
    ...
    assert.ok(holdAfter.x - holdBefore >= 10, "long press must use the three-times-faster repeat rate");
    assert.ok(holdAfter.x - holdBefore <= 13, "long press repeat rate exceeded the intended three-times speed");
